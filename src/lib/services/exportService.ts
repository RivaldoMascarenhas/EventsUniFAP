...
export interface GeneralResultExportItem {
  id: string;
  drawnNumber: number;
  winnerName: string;
  cpf?: string | null;
...
<body>
  ...
  /**
   * Generates CSV string for General Results History
   */
  public static generateAllResultsCsv(items: GeneralResultExportItem[]): string {
    const data = items.map((item) => ({
      "Nº do Bilhete": padNumber(item.drawnNumber, 3),
      "Ganhador": item.winnerName,
      "CPF": maskCPF(item.cpf),
  ...
  /**
   * Generates Excel XLSX Buffer for General Results History
   */
  public static generateAllResultsXlsx(items: GeneralResultExportItem[]): Buffer {
    const data = items.map((item) => ({
      "Nº Bilhete": item.drawnNumber,
      "Ganhador": item.winnerName,
      "CPF": maskCPF(item.cpf),
    ...
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Histórico de Sorteios");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "buffer" });
    return Buffer.from(excelBuffer);
  }

  /**
   * Generates Institutional Printable HTML Report for General Results History
   */
  public static generateAllResultsPrintableHtml(
    items: GeneralResultExportItem[],
    options?: { filterInfo?: string; generatedAt?: Date }
  ): string {
    const generatedAt = options?.generatedAt || new Date();
    const filterInfo = options?.filterInfo || "Todos os eventos registrados";
...
<body>
  <div class="header">
    <div>
      <div class="logo-title">Uni<span>FAP</span> Sorteios</div>
      <div style="font-size: 13px; color: #005088; font-weight: 600; margin-top: 2px;">Centro Universitário Paraíso — Ata Geral Consolidada de Sorteios</div>
    </div>
    <div>
      <span class="badge">Documento Oficial</span>
    </div>
  ...
  <div class="meta-box">
    ...
    <div class="meta-item">
      ...
      <div>${items.length} sorteio(s) registrado(s)</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ${rowsHtml || '<tr><td colspan="6" style="text-align: center; padding: 24px; color: #64748B;">Nenhum sorteio registrado.</td></tr>'}
    </tbody>
  </table>
  ...
  <div class="signatures">
    <div>
      <div class="sig-line">
        <strong>Comissão Organizadora Institucional</strong><br />
        Centro Universitário Paraíso — UniFAP
      </div>
    </div>
    <div>
      <div class="sig-line">
        <strong>Auditoria e Governança de Sistemas</strong><br />
        UniFAP Sorteios
      </div>
    </div>
  </div>
  ...
  /**
   * Generates CSV string for Audit Logs
   */
  public static generateAuditLogsCsv(items: AuditLogExportItem[]): string {
    const data = items.map((item) => ({
      "Data / Hora": formatDateTime(item.timestamp),
      "Usuário": item.userName || "Sistema / Público",
      "E-mail": item.userEmail || "-",
      "Perfil": item.userRole || "-",
      "Ação": item.action,
      "Entidade": item.entity,
      "ID da Entidade": item.entityId || "-",
      "Endereço IP": item.ipAddress || "-",
      "Detalhes / Metadados": item.metadata || "-",
    }));
  ...
  /**
   * Generates Excel XLSX Buffer for Audit Logs
   */
  public static generateAuditLogsXlsx(items: AuditLogExportItem[]): Buffer {
    const data = items.map((item) => ({
      "Data e Hora": formatDateTime(item.timestamp),
      "Usuário": item.userName || "Sistema / Público",
      "E-mail": item.userEmail || "-",
      "Perfil": item.userRole || "-",
      "Ação": item.action,
      "Entidade": item.entity,
      "ID Entidade": item.entityId || "-",
      "IP": item.ipAddress || "-",
  ...
  /**
   * Generates Institutional Printable HTML Report for Audit Logs
   */
  public static generateAuditLogsPrintableHtml(
    items: AuditLogExportItem[],
    options?: { filterInfo?: string; generatedAt?: Date }
  ): string {
    const generatedAt = options?.generatedAt || new Date();
    const filterInfo = options?.filterInfo || "Todos os registros de conformidade";

    const rowsHtml = items
      .map(
        (item, idx) => `
        <tr style="background-color: ${idx % 2 === 0 ? '#FFFFFF' : '#F8FAFC'};">
          <td style="padding: 8px 10px; border-bottom: 1px solid #E2E8F0; color: #475569; font-size: 11px; white-space: nowrap;">
...
<body>
  <div class="header">
    <div>
      <div class="logo-title">Uni<span>FAP</span> Sorteios</div>
      <div style="font-size: 13px; color: #005088; font-weight: 600; margin-top: 2px;">Centro Universitário Paraíso — Relatório Oficial de Auditoria & Conformidade</div>
    </div>
    <div>
      <span class="badge">Trilha de Auditoria</span>
    </div>
  </div>
  ...
  <div class="meta-box">
    ...
    <div class="meta-item">
      ...
      <div>${items.length} evento(s) auditado(s)</div>
    </div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Ação</th>
        <th>Entidade</th>
        <th>Detalhes / Metadados / IP</th>
      </tr>
    </thead>
    <tbody>
      ${rowsHtml || '<tr><td colspan="5" style="text-align: center; padding: 24px; color: #64748B;">Nenhum registro de auditoria encontrado.</td></tr>'}
    </tbody>
  </table>
  ...
  <div class="signatures">
    <div>
      <div class="sig-line">
        <strong>Administrador de Segurança & Governança</strong><br />
        Centro Universitário Paraíso — UniFAP
      </div>
    </div>
    <div>
      <div class="sig-line">
        <strong>Auditoria e Conformidade Institucional</strong><br />
        UniFAP Sorteios
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    Documento de auditoria gerado pelo sistema UniFAP Sorteios em ${formatDateTime(generatedAt)}. Registros assinados eletronicamente e mantidos de forma imutável.
  </div>
